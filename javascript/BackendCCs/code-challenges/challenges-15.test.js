...
const screenForNames = (arr) => {
  const arr2 = [];
  arr.forEach((ele) => {
    if (/^(Mrs|Mr|Dr|Ms)\.\s[A-Z]{1}[a-z]{1,}/.test(ele)) {
      arr2.push(ele);
    }
  });
  return arr2;
...
let biggerThanLuke = (arr) => {
  ...
  const names = arr.reduce((result, character) => {
...
const isSecure = (url) => {
  // Solution code here...
  const regex = /^https:\/\//;
  return regex.test(url) && url.indexOf('https://') === 0;
...
const detectTicTacToeWin = (board) => {
  ...
  const checkWin = (condition) => {
    const [a, b, c] = condition;
    const [rowA, colA] = a;
    const [rowB, colB] = b;
    const [rowC, colC] = c;
    const symbol = board[rowA][colA];
    return symbol !== '' && symbol === board[rowB][colB] && symbol === board[rowC][colC];
  };

  return winConditions.some(checkWin);
};
